...
def helper(item_lst: list, item: object, item_num: int) -> bool:
    """
    A helper function for the helper1 function
    """
    if item_lst[item_num] <= item:
        if item_num < len(item_lst) - 1:
            if item_lst[item_num + 1] > item:
                item_lst.insert(item_num + 1, item)
                return True
        elif item_num == len(item_lst) - 1:
            item_lst.insert(item_num + 1, item)
            return True
...
def helper1(item_lst: list, item: object) -> None:
    """
    A helper function for the add method
    """
    if item < item_lst[0]:
        item_lst.insert(0, item)
    else:
        for item_num in range(len(item_lst)):
            result = helper(item_lst, item, item_num)
            if result:
                break
