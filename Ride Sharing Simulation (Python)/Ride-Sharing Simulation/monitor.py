...
def total_distance(activities: List, distance: float) -> float:
    """
    A helper function for total distance method
    """
    for activity in range(len(activities) - 1):
        m_distance = manhattan_distance(
            activities[activity].location,
            activities[activity + 1].location)
        distance += m_distance
    return distance


def ride_distance(activities: List, distance: float) -> float:
    """
    A helper function for ride distance method
    """
    for activity in range(len(activities) - 1):
        if activities[activity].description == PICKUP:
            m_distance = manhattan_distance(
                activities[activity].location,
                activities[activity + 1].location)
            distance += m_distance
    ...
    def _average_total_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            if len(activities) <= 1:
                ...
                distance = total_distance(activities, distance)
                count += 1
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            if len(activities) <= 2:
                ...
                distance = ride_distance(activities, distance)
                count += 1
